#version 120
uniform sampler2DRect s_texture;
void main(){
	//this is the fragment shader
	//this is where the pixel level drawing happens
	//gl_FragCoord gives us the x and y of the current pixel its drawing
	
	//we grab the x and y and store them in an int
	float xVal = gl_FragCoord.x;
	float yVal = gl_FragCoord.y;
//	vec2 v = vec2(fract(sin(dot(gl_FragCoord.xy ,vec2(12.9898,78.233))) * 43758.5453),fract(sin(dot(gl_FragCoord.xy ,vec2(12.9898,78.233))) * 43758.5453));
    
	//we use the mod function to only draw pixels if they are every 2 in x or every 4 in y
//	if( mod(xVal, 2.0) == 0.5 && mod(yVal, 4.0) == 0.5 ){
		gl_FragColor = texture2DRect( s_texture,  gl_TexCoord[0].st);
    //vec2( xVal/1680.0, yVal/1050.0)
    //gl_Color
    //    texture2D( samplerForMyTexture, vec2( x, y ) );
//    }else{
//		gl_FragColor.a = 0.0;
//	}
	
}